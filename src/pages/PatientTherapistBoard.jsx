...
import { Clock, DollarSign, MessageSquare, Circle, UserCheck, RefreshCw, XCircle, CheckCircle, Info } from 'lucide-react';
const TherapistCard = ({ therapist, schedules, onlineUsers, onSendChangeRequest }) => {
  const isOnline = onlineUsers[therapist.id] === 'online'; 
...
      <button
        onClick={() => onSendChangeRequest(therapist.id)} // Pass the therapist's UUID
...
    setupInitialData();
  }, [fetchTherapists, fetchSchedules, fetchTherapistNotifications]);
  useEffect(() => {
    if (!userId) return; 
...
        
        Object.keys(newState).forEach(therapistId => {
            onlineStatusMap[therapistId] = 'online';
        });
        setOnlineUsers(onlineStatusMap);
      })
      .on('presence', { event: 'leave' }, ({ key, currentPresences }) => {
        if (currentPresences.length === 0) {
            setOnlineUsers(prev => ({ ...prev, [key]: 'offline' }));
        }
      })
      .subscribe(async (status) => {
        if (status === 'SUBSCRIBED' && userId) {
            await presenceChannel.track({ user_id: userId, role: 'patient' });
        }
      });
    return () => {
